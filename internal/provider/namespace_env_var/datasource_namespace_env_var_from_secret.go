...
var (
	_ datasource.DataSource              = &namespaceEnvVarFromSecretDataSource{}
	_ datasource.DataSourceWithConfigure = &namespaceEnvVarFromSecretDataSource{}
)
...
func (d *namespaceEnvVarFromSecretDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_namespace_env_var_from_secret"
}

func (d *namespaceEnvVarFromSecretDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: `Namespace Inputs (Env Vars) --- Retrieves a Namespace Env Var (From Secret) from Snap CD.`,
		Attributes: map[string]schema.Attribute{
			...
			"namespace_id": schema.StringAttribute{
				Required:    true,
				Description: DescSharedNamespaceId1 + "Namespace Env Var (From Secret)" + DescSharedNamespaceId2,
			},
		},
	}
}

func (d *namespaceEnvVarFromSecretDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *namespaceEnvVarFromSecretDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if httpError != nil {
		resp.Diagnostics.AddError(namespaceEnvVarFromSecretDefaultError, "Error calling GET: "+httpError.Error.Error())
		return
	}
