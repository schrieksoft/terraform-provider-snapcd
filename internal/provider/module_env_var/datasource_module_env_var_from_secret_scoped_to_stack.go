...
func (d *moduleEnvVarFromSecretScopedToStackDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	result, httpError := d.client.Get(fmt.Sprintf("%s/%s/%s", moduleEnvVarFromSecretScopedToStackEndpoint, data.ModuleId.ValueString(), data.SecretScopedToStackId.ValueString()))
	var err error
	if httpError != nil {
		err = httpError.Error
	} else {
