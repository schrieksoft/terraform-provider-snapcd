...
var ModuleEnvVarFromSecretCreateConfig = secret.SimpleSecretCreateConfig + providerconfig.AppendRandomString(`
...
func TestAccResourceModuleEnvVarFromSecret_CreateUpdate(t *testing.T) {
	resource.UnitTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: providerconfig.ProviderConfig + core.ModuleCreateConfig + secret.SimpleSecretCreateConfig + providerconfig.AppendRandomString(`
resource "snapcd_module_env_var_from_secret" "this" { 
  module_id = snapcd_module.this.id
  name  = "somevalue%s"
  secret_id = snapcd_simple_secret.this.id
  type = "NotString"
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrSet("snapcd_module_env_var_from_secret.this", "id"),
					resource.TestCheckResourceAttr("snapcd_module_env_var_from_secret.this", "type", "NotString"),
				),
			},
		},
	})
